...
def save_im(writer, loader, model, device, epoch, title):
    #tensorboard: save images on first batch of training/validation set (taken randomly among all validation images)
    ...
    writer.add_image(title+'/original', im, epoch)
    ...
    writer.add_image(title+'/reconstructed', im, epoch)
...
def begin_epoch_stats(writer, train_loader, val_loader, model, criterion, optimizer, device, epoch):
    ...
    save_im(writer, train_loader, model, device, epoch, 'train')
    save_im(writer, val_loader, model, device, epoch, 'val')